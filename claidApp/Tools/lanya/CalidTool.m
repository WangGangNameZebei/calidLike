...
+ (NSString *)crc32producedataStr:(NSString *)dataStr{
    ...
    numberKey = ctcrcs32(key2,dataStr.length/2);
    key1[0] = numberKey>>24;
    key1[1] = numberKey>>16;
    key1[2] = numberKey>>8;
    key1[3] = numberKey;
...
+ (NSString *)payByCardInstructionsActionString:(NSString *)string {
    ...
    if([[string substringWithRange:NSMakeRange(22, 2)]   isEqualToString:@"01"]){       //设置卡
        ...
        if ([[baseVC userInfoReaduserkey:@"setupNumber"] isEqualToString:@"0000"] || [[baseVC userInfoReaduserkey:@"setupNumber"] isEqualToString:@""]){  //无随机数存储
            aa =1000+rand()%(9999 - 1000 + 1);
            [baseVC userInfowriteuserkey:@"setupNumber" uservalue:[NSString stringWithFormat:@"%ld",(long)aa]]; //修改随机数
            ...
            string = [NSString stringWithFormat:@"%@%@",string,[baseVC userInfoReaduserkey:@"setupNumber"]];
...
#pragma mark - 发送数据加密
+ (NSString *)visitorlanyaSendoutDataAction:(NSString *)data {
    unsigned int numberKey;
    unsigned char key1[4];
    unsigned char key2[100];
    NSString * TheTwoCharacters;
    ...
    return [NSString stringWithFormat:@"aa%@",strTow];

}
#pragma mark - 发送数据加密
+ (NSString *)lanyaSendoutDataAction:(NSString *)data {
    unsigned int numberKey;
    unsigned char key1[4];
    unsigned char key2[100];
    NSString * TheTwoCharacters;
    ...
    NSString *lanyaDataStr = [AESCrypt decrypt:[[BaseViewController alloc] userInfoReaduserkey:@"lanyaAESData"] password:AES_PASSWORD];
    strOne = [lanyaDataStr substringWithRange:NSMakeRange(0,104)];
    strTow = [self jiamiaTostringAcction:strOne numberKey:numberKey];
    strTow = [NSString stringWithFormat:@"%@%@",strTow,[self jiamiaTostringAcction:[lanyaDataStr substringWithRange:NSMakeRange(104, 104)] numberKey:numberKey]];
    return [NSString stringWithFormat:@"aa%@",strTow];
...
unsigned int ctcrcs32( unsigned char buf[], unsigned char len)
{
    unsigned int ret = 0x13141516;
    int i;
    for(i = 0; i < len;i++)
    {
        ret = ctCRC32_table[((ret & 0xFF) ^ buf[i])] ^ (ret >> 8);
...
unsigned int ctCRC32_table[256] =
{
...
void ctxor(unsigned char key[]) {
    ...
    {
        if (str % klen == 0 ){
            cttmpstr[j]=(numberOne[j]^key[j%klen]) ^ str;
        }else if (str % klen == 1) {
            cttmpstr[j]=(numberOne[j]^key[j%klen])^(~(str)) ;
        }else if (str % klen == 2) {
            cttmpstr[j]=numberOne[j]^key[j%klen];
        } else {
            cttmpstr[j]=~((numberOne[j]^key[j%klen]) ^ str);
        }
        if(!cttmpstr[j])
            cttmpstr[j]=numberOne[j];
    }
